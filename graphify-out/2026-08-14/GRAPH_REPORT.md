# Graph Report - petakeu  (2026-08-14)

## Corpus Check
- 564 files · ~521,830 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4753 nodes · 6224 edges · 536 communities (349 shown, 187 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 106 edges (avg confidence: 0.59)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `fce04da1`
...
## Communities (536 total, 187 thin omitted)
...
Cohesion: 0.09
Nodes (31): rankings, renderPdf(), rows, addMonths(), applyHeaderStyle(), decodeSafeLogo(), DEFAULT_REPORT_BRANDING, fetchReportData() (+23 more)
...
Cohesion: 0.15
Nodes (13): devDependencies, eslint-config-prettier, eslint-plugin-import, @types/jsonwebtoken, @types/node-cron, @types/swagger-jsdoc, @typescript-eslint/parser, eslint-config-prettier (+5 more)
...
Cohesion: 0.08
Nodes (45): getPgPool(), instrumentQuery(), getCached(), invalidateCacheByPrefix(), startMvRefreshCron(), EXPECTED_HEADERS, findingToError(), isFuturePeriod() (+37 more)
...
Cohesion: 0.14
Nodes (29): options, setupSwagger(), swaggerSpec, getRedisClient(), getReportQueue(), getUploadQueue(), createApp(), env (+21 more)
...
### Community 203 - "storage-service.ts"
Cohesion: 0.22
Nodes (14): ensureBucket(), getPresignedDownloadUrl(), getS3Client(), uploadStreamToS3(), uploadToS3(), MockPgPool, MockQuery, QueryCall (+6 more)
...
Cohesion: 0.06
Nodes (45): mockQuery, mockRedisConnect, mockRedisDel, mockRedisGet, mockRedisKeys, mockRedisOn, mockRedisSetEx, mockRedisGet (+37 more)
...
### Community 441 - "Table Definitions"
Cohesion: 0.18
...
## Knowledge Gaps
- **2641 isolated node(s):** `mockQuery`, `mockRedisGet`, `mockRedisSetEx`, `mockRedisKeys`, `mockRedisDel` (+2636 more)
...
- **Why does `vitest` connect `redis.ts` to `server.ts`, `report-worker.ts`, `compilerOptions`, `scheduled-report-cron.test.ts`, `upload-pipeline.integration.test.ts`, `storage-service.ts`, `middleware/auth.ts`, `validators/report.ts`, `upload-validation.ts`, `createReportEmailDispatcher`, `App.tsx`, `validators/analytics.ts`, `UploadPage.tsx`, `client.ts`, `Legend.tsx`, `AuditLogInspector.tsx`, `getPgPool`, `AnalyticsPage.tsx`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `getPgPool()` connect `getPgPool` to `server.ts`, `report-worker.ts`, `approval-service.ts`, `scheduled-report-service.ts`, `buildRankingQuery`, `upload-pipeline.integration.test.ts`, `storage-service.ts`, `src/index.ts`, `redis.ts`, `upload-service.ts`, `analytics-service.ts`, `buildOverview`, `alert-service.ts`, `report-controller.ts`?**
